...
class UpComingMovieSeeAllViewModel: ViewModel(),MovieListDelegate {

    private val mModel: HmyhAssignmentThreeModel = HmyhAssignmentThreeModelImpl

    private var upComingMovie: MutableLiveData<UpComingMovieVO> = MutableLiveData()
    ...
    fun loadMoreUpComingMovie(){
        ...
        upComingMovie.value?.page?.let {
            page = it
        }
        upComingMovie.value?.totalPage?.let {
            totalPage = it
        }
    ...
    fun getUpComingMovie(): MutableLiveData<List<MovieListVO>>{
